...
const { notifyAdminOfError } = require('./_lib/admin-error-notify.cjs')
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { data: inserted, error } = await supabase.from('error_reports').insert({
      user_id,
      user_email,
    ...
    await notifyAdminOfError(supabase, inserted)
