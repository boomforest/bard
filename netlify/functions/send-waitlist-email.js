...
// POST body: { event_id, message, origin? }
...
const renderHtml = ({ eventName, message, eventUrl, recipientName }) => `
  <div style="font-family: -apple-system, system-ui, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; background: #08080c; color: #e8e0d0;">
    ...
    <div style="background: #12121a; border: 1px solid #1e1e2a; border-radius: 14px; padding: 24px; margin-bottom: 20px;">
      <div style="font-size: 11px; color: #aaff00; text-transform: uppercase; letter-spacing: 0.15em; font-weight: 700; margin-bottom: 8px;">Tickets opened up</div>
      <div style="font-size: 22px; font-weight: 900; color: #e8e0d0; letter-spacing: -0.02em; margin-bottom: 12px;">${escapeHtml(eventName)}</div>
      <div style="color: #c8c0d0; font-size: 14px; line-height: 1.6; white-space: pre-wrap; margin-bottom: 16px;">${escapeHtml(recipientName ? `Hi ${recipientName},\n\n` : '')}${escapeHtml(message)}</div>
      <p style="margin: 14px 0 0;">
        <a href="${eventUrl}" style="display: inline-block; background: linear-gradient(135deg, #dd22aa 0%, #f07020 100%); color: #000; font-weight: 800; padding: 12px 24px; border-radius: 10px; text-decoration: none; font-size: 14px;">Grab a ticket →</a>
      </p>
    </div>

    <div style="text-align: center; color: #3a3448; font-size: 11px; letter-spacing: 0.05em;">
      Powered by GRAIL · grail.mx<br>
      <span style="color: #8a8098;">You're getting this because you joined the waitlist for this event.</span>
    </div>
  </div>
`
...
exports.handler = async (event) => {
  ...
  try {
    const { event_id, message, origin } = JSON.parse(event.body || '{}')
    ...
    const subject = `Tickets opened up — ${eventName}`
